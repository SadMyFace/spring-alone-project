...
<br>
	...
	<c:set value="${userEmail }" var="authEmail"></c:set>
	<table class="table">
		<tr>
			<th>Bno</th>
			<td>${bvo.bno }</td>
  			...
  			<li class="list-group-item">
  				<c:choose>
  					<c:when test="${fvo.fileType == 1 }">
  						<div>
  							<a href="/upload/${fvo.saveDir }/${fvo.uuid}_th_${fvo.fileName}" download><img alt="" src="/upload/${fvo.saveDir }/${fvo.uuid}_th_${fvo.fileName}"></a>
  						</div>
  					</c:when>
  					<c:otherwise>
  						<div>
	...
	<!-- 댓글 등록 라인 -->
	<!-- 게시물 작성자인지 비교후 댓글 등록 라인 표시 여부 결정 -->
	<c:choose>
		<c:when test="${authEmail eq null }">
